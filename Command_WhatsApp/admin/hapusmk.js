...
module.exports = {
    ...
    execute: async (hydro, m, args, text, { isAdminBot, prefix }) => {
        ...
        let rows = global.db.mk_si_2025.map((mk, i) => ({
            header: `ID: ${i + 1}`, title: mk.singkatan, description: `${mk.hari} | ${mk.jam}`, id: `${prefix}delmk_conf ${i} ${m.sender}`
        }));
        
        let msg = generateWAMessageFromContent(m.chat, {
            viewOnceMessage: { message: { messageContextInfo: { deviceListMetadata: {}, deviceListMetadataVersion: 2 },
                interactiveMessage: {
                    body: { text: `Pilih Mata Kuliah yang ingin kamu *HAPUS*  ` },
                    nativeFlowMessage: { buttons: [{ name: "single_select", buttonParamsJson: JSON.stringify({ title: "PILIH MK", sections: [{ title: "Daftar Mata Kuliah", rows: rows }] }) }]}
                }
            }}
        }, { quoted: m });
