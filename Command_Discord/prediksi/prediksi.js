...
module.exports = {
    ...
    async execute(message, args) {
        ...
        const sent = await message.reply('_⏳ Memproses Machine Learning SVR (Support Vector Regression) dari Awal (From Scratch) pakai Javascript Murni..._');
        ...
        try {
            ...
            if (!fs.existsSync(csvPath)) {
                return sent.edit('❌ File traffic_log.csv tidak ditemukan.');
            }
            ...
            if (n <= 1) return sent.edit('❌ Data masih kosong atau kurang dari 2 hari untuk diprediksi.');
            ...
            txt += `_📌 Ketik **!prediksidoc** untuk download Laporan PDF (Plot by Python)_`;

            await sent.edit(txt);

        } catch (e) {
            console.error(e);
            await sent.edit(`❌ Terjadi error saat memproses JS SVR: ${e.message}`);
        }
    }
};
