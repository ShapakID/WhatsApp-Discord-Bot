...
const DB_PATH = path.resolve(__dirname, '../../database/cve_channels.json');
const LAST_SEEN_PATH = path.resolve(__dirname, '../../database/cve_last_seen.json');
        ...
        const checkCVE = async () => {
            ...
            try {
                ...
                if (e.response && (e.response.status === 503 || e.response.status === 502)) {
                    ...
                    // console.error(`[PingCVE] API server sedang maintenance (503). Retrying later...`);
                } else {
