...
if (!global.db) {
    if (fs.existsSync('./Data/database.json')) {
        global.db = JSON.parse(fs.readFileSync('./Data/database.json', 'utf-8'))
    } else {
...
module.exports = hydro = async (hydro, m, chatUpdate, store) => {
    try {
        ...
        if (fs.existsSync('./Data/owner.json')) {
            let extraOwner = JSON.parse(fs.readFileSync('./Data/owner.json'));
...
🚀 *Pᴏᴡᴇʀᴇᴅ Bʏ ${global.botname}*`)
            ...
            case 'addowner': {
                ...
                let extraOwner = fs.existsSync('./Data/owner.json') ? JSON.parse(fs.readFileSync('./Data/owner.json')) : []
                if (!extraOwner.includes(num)) extraOwner.push(num)
                fs.writeFileSync('./Data/owner.json', JSON.stringify(extraOwner, null, 2))
            ...
            case 'addbadword': {
                ...
                if (!fs.existsSync('./Data/badword.json')) fs.writeFileSync('./Data/badword.json', JSON.stringify([]));
                let bw = JSON.parse(fs.readFileSync('./Data/badword.json'));

                let word = text.toLowerCase().trim();
                if (bw.includes(word)) return replytolak(`⚠️ Kata *${word}* sudah ada di dalam database toxic!`);

                bw.push(word);
                fs.writeFileSync('./Data/badword.json', JSON.stringify(bw, null, 2));
            ...
            case 'listbadword': {
                ...
                if (!fs.existsSync('./Data/badword.json')) {
            ...
            case 'addrole': {
                ...
                fs.writeFileSync('./Data/roles.json', JSON.stringify(global.dbRole, null, 2));
