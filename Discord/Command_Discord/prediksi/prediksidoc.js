...
module.exports = {
    ...
    async execute(message, args) {
        ...
        if (!isOwner) {
            return message.reply('⛔ Fitur ini hanya untuk Owner bot.');
        }

        const logFile = path.join(__dirname, '../../traffic_log.csv');
        if (!fs.existsSync(logFile)) {
            return message.reply('Belum ada data trafik yang terkumpul.');
        }

        const scriptPath = path.join(__dirname, '../../Command_WhatsApp/prediksi/predict_svr.py');
        let stbLimit = 1000;
        let filterDate = null;
        ...
        const sent = await message.reply(filterDate ? `_⏳ Memfilter data untuk tanggal **${filterDate}**..._` : `_⏳ Sedang menyiapkan Dokumen Laporan (PDF) dan Backup Data (CSV)..._`);
        ...
        if (rowCount === 0 && filterDate) {
            return sent.edit(`⚠️ Tidak ada log trafik untuk tanggal ${filterDate}.`);
        }
        ...
        const excelFileName = filterDate ? `Dataset_Trafik_${filterDate}.xlsx` : 'Dataset_Trafik_Bot_Rapih.xlsx';
        const excelAttachment = new AttachmentBuilder(excelBuffer, { name: excelFileName });
        ...
        const caption = filterDate
            ...
            : `📂 **BACKUP DATA RIIL**\nIni adalah file dataset dari log trafik yang sudah dikemas jadi file Excel resmi.\n_💡 Tombol Filter sudah otomatis aktif di setiap kolom._`;

        await message.channel.send({ content: caption, files: [excelAttachment] });
        ...
        exec(`python3 "${scriptPath}" ${stbLimit}`, (error, stdout, stderr) => {
            if (error) {
                exec(`python "${scriptPath}" ${stbLimit}`, (err2, stdout2, stderr2) => {
                    if (err2) {
                        return sent.edit(`❌ Gagal memproses Python ML untuk PDF.\nError: ${err2.message}`);
                    }
                    generatePdf(stdout2);
                });
                return;
        ...
        function generatePdf(outputStr) {
            try {
                ...
                doc.on('end', () => {
                    ...
                    const pdfAttachment = new AttachmentBuilder(resultBuffer, { name: 'Laporan_Prediksi_SVR_Bot.pdf' });
                    message.channel.send({
                        content: '📄 **LAPORAN PDF SELESAI**\n\nSiap dilampirkan untuk tugasmu! (Proses menggunakan Python Scikit-Learn)',
                        files: [pdfAttachment]
                    });
                    sent.edit('✅ Semua dokumen berhasil dibuat!');
                });
                ...
                sent.edit(`Gagal membuat dokumen PDF: ${err.message}`);
            }
        }
    }
};
