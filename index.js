...
const { Boom } = boom
const { default: makeWASocket, BufferJSON, processedMessages, PHONENUMBER_MCC, initInMemoryKeyStore, DisconnectReason, AnyMessageContent,
    makeInMemoryStore, useMultiFileAuthState, delay, fetchLatestBaileysVersion, generateForwardMessageContent,
...
async function hydroInd() {
    ...
    hydro.ev.on('group-participants.update', async (anu) => {
        try {
            await require('./lib/group').participantsUpdate(hydro, anu);
        } catch (e) {
            console.log(chalk.red('Abaikan, error cek metadata grup (403 Forbidden)'));
        }
    });

    hydro.ev.on('groups.update', async (anu) => {
        try {
            await require('./lib/group').groupsUpdate(hydro, anu, store);
        } catch (e) {
            console.log(chalk.red('Abaikan, error update grup'));
        }
    });
